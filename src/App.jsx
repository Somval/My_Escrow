...
/* ═══ ROOT — replace your existing export default function App() with this ═══════════════════════════════════════════════ */
export default function App(){
  const [page,setPage]=useState("splash");
  const [user,setUser]=useState(null);
  const [pendingUser,setPendingUser]=useState(null); // holds user while OTP is pending

  const navigate=p=>{setPage(p);window.scrollTo(0,0);};

  // Called by SignupPage after form validation passes
  const onSignupComplete=u=>{
    setPendingUser(u);
    navigate("otp");           // go to OTP screen instead of dashboard directly
  };

  // Called by OTPPage after code verified
  const onOTPSuccess=()=>{
    setUser(pendingUser);
    setPendingUser(null);
    navigate("dashboard");
  ...
  // Called by LoginPage (no OTP needed for login in this flow)
  ...
  if(page==="login"){
    return(<><style>{CSS}</style><LoginPage onSuccess={onLoginSuccess} navigate={navigate}/></>);
  }
  if(page==="signup"){
    return(<><style>{CSS}</style><SignupPage onSuccess={onSignupComplete} navigate={navigate}/></>);
  }
  if(page==="otp"){
    return(<><style>{CSS}</style><OTPPage email={pendingUser?.email} onSuccess={onOTPSuccess} navigate={navigate}/></>);
  }
  if(page==="forgot"){
    return(<><style>{CSS}</style><ForgotPasswordPage navigate={navigate}/></>);
  }
  if(page==="dashboard"&&user){
    return(<><style>{CSS}</style><Dashboard user={user} onLogout={()=>{setUser(null);setPendingUser(null);navigate("home");}} navigate={navigate}/></>);
  }
  return(<>
    <style>{CSS}</style>
